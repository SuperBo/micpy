...
static void
get_array_memory_extents(PyMicArrayObject *arr,
                         npy_uintp *out_start, npy_uintp *out_end,
                         npy_uintp *num_bytes)
{
    npy_intp low, upper;
    int j;
    offset_bounds_from_strides(PyMicArray_ITEMSIZE(arr), PyMicArray_NDIM(arr),
                               PyMicArray_DIMS(arr), PyMicArray_STRIDES(arr),
                               &low, &upper);
    *out_start = (npy_uintp)PyMicArray_DATA(arr) + (npy_uintp)low;
    *out_end = (npy_uintp)PyMicArray_DATA(arr) + (npy_uintp)upper;

    *num_bytes = PyMicArray_ITEMSIZE(arr);
    for (j = 0; j < PyMicArray_NDIM(arr); ++j) {
        *num_bytes *= PyMicArray_DIM(arr, j);
    }
}


static int
strides_to_terms(PyMicArrayObject *arr, diophantine_term_t *terms,
                 unsigned int *nterms, int skip_empty)
{
    unsigned int i;

    for (i = 0; i < PyMicArray_NDIM(arr); ++i) {
        if (skip_empty) {
            if (PyMicArray_DIM(arr, i) <= 1 || PyMicArray_STRIDE(arr, i) == 0) {
                continue;
            }
        }

        terms[*nterms].a = PyMicArray_STRIDE(arr, i);
        ...
        terms[*nterms].ub = PyMicArray_DIM(arr, i) - 1;
        ++*nterms;
    }
 ...
 */
NPY_VISIBILITY_HIDDEN mem_overlap_t
solve_may_share_memory(PyMicArrayObject *a, PyMicArrayObject *b,
                       Py_ssize_t max_work)
{
    npy_int64 rhs;
    ...
    */
    ...
    if (PyMicArray_ITEMSIZE(a) > 1) {
        terms[nterms].a = 1;
        terms[nterms].ub = PyMicArray_ITEMSIZE(a) - 1;
        ++nterms;
    }
    if (PyMicArray_ITEMSIZE(b) > 1) {
        terms[nterms].a = 1;
        terms[nterms].ub = PyMicArray_ITEMSIZE(b) - 1;
        ++nterms;
    }
 ...
 */
NPY_VISIBILITY_HIDDEN mem_overlap_t
solve_may_have_internal_overlap(PyMicArrayObject *a, Py_ssize_t max_work)
{
