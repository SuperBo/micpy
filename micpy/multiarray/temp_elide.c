...
#if defined(NPY_OS_LINUX) || defined(NPY_OS_BSD) || defined(NPY_OS_DARWIN)
 ...
 */
static int
can_elide_temp(PyMicArrayObject * alhs, PyObject * orhs, int * cannot)
{
    /*
     ...
     */
    if (Py_REFCNT(alhs) != 1 || !PyMicArray_CheckExact(alhs) ||
            !PyMicArray_ISNUMBER(alhs) ||
            !(PyMicArray_FLAGS(alhs) & NPY_ARRAY_OWNDATA) ||
            PyMicArray_NBYTES(alhs) < NPY_MIN_ELIDE_BYTES) {
        return 0;
    }
    if (PyMicArray_CheckExact(orhs) || PyArray_CheckAnyScalar(orhs)) {
        PyMicArrayObject * arhs;

        /* create array from right hand side */
        Py_INCREF(orhs);
        arhs = (PyMicArrayObject *)PyMicArray_EnsureArray(orhs,
                    PyMicArray_DEVICE(alhs));
        if (arhs == NULL) {
            return 0;
        }
         ...
         */
        if (!(PyMicArray_NDIM(arhs) == 0 ||
              (PyMicArray_NDIM(arhs) == PyMicArray_NDIM(alhs) &&
               PyArray_CompareLists(PyMicArray_DIMS(alhs), PyMicArray_DIMS(arhs),
                                    PyMicArray_NDIM(arhs))))) {
                Py_DECREF(arhs);
                return 0;
        }

        /* must be safe to cast (checks values for scalar in rhs) */
        if (PyMicArray_CanCastArrayTo(arhs, PyMicArray_DESCR(alhs),
                                   NPY_SAFE_CASTING)) {
            Py_DECREF(arhs);
                 ...
                 PyObject ** res, int commutative)
{
    ...
    if (can_elide_temp(m1, m2, &cannot)) {
        *res = inplace_op(m1, m2);
        ...
        if (can_elide_temp((PyMicArrayObject *)m2, (PyObject *)m1, &cannot)) {
            *res = inplace_op((PyMicArrayObject *)m2, (PyObject *)m1);
#if NPY_ELIDE_DEBUG != 0
...
{
    int cannot;
    if (Py_REFCNT(m1) != 1 || !PyMicArray_CheckExact(m1) ||
            !PyMicArray_ISNUMBER(m1) ||
            !(PyMicArray_FLAGS(m1) & NPY_ARRAY_OWNDATA) ||
            PyMicArray_NBYTES(m1) < NPY_MIN_ELIDE_BYTES) {
        return 0;
    }
