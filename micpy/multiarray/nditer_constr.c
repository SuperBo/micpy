...
#ifndef NPY_ITER_COPY_IF_OVERLAP
#define NPY_ITER_COPY_IF_OVERLAP            0x00002000
#endif

/* Internal helper functions private to this file */
static int
mpyiter_get_common_device(int *device, int nop, PyMicArrayObject **ops);
static int
...
npyiter_check_op_axes(int nop, int oa_ndim, int **op_axes,
                        npy_intp *itershape);
static int
npyiter_calculate_ndim(int nop, PyMicArrayObject **op_in,
                       int oa_ndim);
static int
npyiter_check_per_op_flags(npy_uint32 flags, npyiter_opitflags *op_itflags);
static int
npyiter_prepare_one_operand(PyMicArrayObject **op,
                        char **op_dataptr,
...
npyiter_prepare_operands(int nop,
                    ...
                    npy_int8 *out_maskop);
static int
npyiter_check_casting(int nop, PyMicArrayObject **op,
                    PyArray_Descr **op_dtype,
                    NPY_CASTING casting,
                    npyiter_opitflags *op_itflags);
static int
npyiter_fill_axisdata(MpyIter *iter, npy_uint32 flags, npyiter_opitflags *op_itflags,
                    char **op_dataptr,
                    npy_uint32 *op_flags, int **op_axes,
                    npy_intp *itershape);
static void
npyiter_replace_axisdata(MpyIter *iter, int iop,
                      PyMicArrayObject *op,
                      int op_ndim, char *op_dataptr,
                      int *op_axes);
static void
npyiter_compute_index_strides(MpyIter *iter, npy_uint32 flags);
static void
npyiter_apply_forced_iteration_order(MpyIter *iter, NPY_ORDER order);
static void
npyiter_flip_negative_strides(MpyIter *iter);
static void
npyiter_reverse_axis_ordering(MpyIter *iter);
static void
npyiter_find_best_axis_ordering(MpyIter *iter);
static PyArray_Descr *
npyiter_get_common_dtype(int nop, PyMicArrayObject **op,
                        npyiter_opitflags *op_itflags, PyArray_Descr **op_dtype,
                        PyArray_Descr **op_request_dtypes,
                        int only_inputs);
static PyMicArrayObject *
npyiter_new_temp_array(MpyIter *iter, PyTypeObject *subtype,
                npy_uint32 flags, npyiter_opitflags *op_itflags,
                int op_ndim, npy_intp *shape,
                PyArray_Descr *op_dtype, int *op_axes);
static int
npyiter_allocate_arrays(MpyIter *iter,
                        npy_uint32 flags,
                        PyArray_Descr **op_dtype, PyTypeObject *subtype,
                        npy_uint32 *op_flags, npyiter_opitflags *op_itflags,
                        int **op_axes);
static void
npyiter_get_priority_subtype(int nop, PyMicArrayObject **op,
                            npyiter_opitflags *op_itflags,
                            double *subtype_priority, PyTypeObject **subtype);
static int
npyiter_allocate_transfer_functions(MpyIter *iter);
                 ...
                 npy_intp buffersize)
{
    ...
    PyTypeObject *subtype = &PyMicArray_Type;
    ...
    //TODO: timeit
    /* Check common device */
    if (!mpyiter_get_common_device(&device, nop, op_in)) {
        PyErr_SetString(PyExc_ValueError, "Operands must be on one device");
        return NULL;
    }
    ...
    NIT_NDIM(iter) = ndim;
    NIT_NOP(iter) = nop;
    NIT_DEVICE(iter) = device;
    NIT_OFFITER(iter) = offiter;
    NIT_MASKOP(iter) = -1;
    ...
    /*                                  NPY_ITEM_IS_POINTER | */
    /*                                  NPY_NEEDS_PYAPI)) != 0) { */
    /*             /1* Iteration needs API access *1/ */
    ...
    if (itflags & NPY_ITFLAG_BUFFER) {
        ...
        if (!(itflags & NPY_ITFLAG_DELAYBUF)) {
            ...
            if (!mpyiter_allocate_buffers(iter, NULL)) {
                MpyIter_Deallocate(iter);
                return NULL;
            }
...
MpyIter_MultiNew(int nop, PyMicArrayObject **op_in, npy_uint32 flags,
                 ...
                 PyArray_Descr **op_request_dtypes)
{
    return MpyIter_AdvancedNew(nop, op_in, flags, order, casting,
                  ...
                  PyArray_Descr* dtype)
{
    ...
    return MpyIter_AdvancedNew(1, &op, flags, order, casting,
                            &op_flags, &dtype,
                            -1, NULL, NULL, 0);
}
...
MpyIter_Copy(MpyIter *iter)
{
    ...
    if (itflags & NPY_ITFLAG_BUFFER) {
        //TODO: this shouble be on mic device
        ...
        if (!out_of_memory && NBF_SIZE(bufferdata) > 0) {
            mpyiter_goto_iterindex(newiter, NIT_ITERINDEX(newiter));

            /* Prepare the next buffers and set iterend/size */
            mpyiter_copy_to_buffers(newiter, NULL);
        }
    }

    if (out_of_memory) {
        MpyIter_Deallocate(newiter);
        PyErr_NoMemory();
        return NULL;
    }
...
/* Get common device from PyMicArrayObjects
 *
 * Retunrs 1 on success, 0 on error
 */
static int
mpyiter_get_common_device(int *device, int nop, PyMicArrayObject **ops)
{
    ...
    int _device = PyMicArray_DEVICE(ops[0]);
    int i;

    for (i = 1; i < nop; i++) {
        if (ops[i] != NULL && PyMicArray_DEVICE(ops[i]) != _device) {
            return 0;
        }
    }
                       ...
                       int oa_ndim)
{
    ...
    /* Otherwise it's the maximum 'ndim' from the operands */
        ...
        for (iop = 0; iop < nop; ++iop) {
            if (op_in[iop] != NULL) {
                int ondim = PyMicArray_NDIM(op_in[iop]);
                if (ondim > ndim) {
                    ndim = ondim;
                }
            }
                        ...
                        npy_uint32 op_flags, npyiter_opitflags *op_itflags)
{
    ...
    if (PyMicArray_Check(*op)) {
        ...
        if ((*op_itflags) & NPY_OP_ITFLAG_WRITE
            && PyMicArray_FailUnlessWriteable(*op, "operand array with iterator "
                                           "write flag set") < 0) {
            return 0;
        }
        if (!(flags & NPY_ITER_ZEROSIZE_OK) && PyMicArray_SIZE(*op) == 0) {
            PyErr_SetString(PyExc_ValueError,
                    "Iteration of zero-sized operands is not enabled");
            return 0;
        }
        *op_dataptr = PyMicArray_BYTES(*op);
        /* PyArray_DESCR does not give us a reference */
        *op_dtype = PyMicArray_DESCR(*op);
        if (*op_dtype == NULL) {
            PyErr_SetString(PyExc_ValueError,
         ...
         */
        if (!(flags & NPY_ITER_REFS_OK)) {
            PyArray_Descr *dt = PyMicArray_DESCR(*op);
        ...
        if (op_request_dtype != NULL) {
            ...
            PyMicArray_AdaptFlexibleDType((PyObject *)(*op), PyMicArray_DESCR(*op),
                                        &op_request_dtype);
            if (op_request_dtype == NULL) {
                return 0;
            }
 ...
 */
static int
npyiter_prepare_operands(int nop, PyMicArrayObject **op_in,
                    PyMicArrayObject **op,
                    char **op_dataptr,
                    PyArray_Descr **op_request_dtypes,
                    PyArray_Descr **op_dtype,
                    ...
                    npyiter_opitflags *op_itflags)
{
    ...
    for(iop = 0; iop < nop; ++iop) {
        ...
        printf("op: ");
        if (op[iop] != NULL) {
            PyObject_Print((PyObject *)PyMicArray_DESCR(op[iop]), stdout, 0);
        }
        else {
            printf("<null>");
        }
        ...
        if (op[iop] != NULL && !PyArray_EquivTypes(PyMicArray_DESCR(op[iop]),
                                                     op_dtype[iop])) {
            /* Check read (op -> temp) casting */
            if ((op_itflags[iop] & NPY_OP_ITFLAG_READ) &&
                        !PyMicArray_CanCastArrayTo(op[iop],
                                          op_dtype[iop],
                                          casting)) {
                PyObject *errmsg;
                ...
                PyUString_ConcatAndDel(&errmsg,
                        PyObject_Repr((PyObject *)PyMicArray_DESCR(op[iop])));
                PyUString_ConcatAndDel(&errmsg,
...
//TODO:???
static int
check_mask_for_writemasked_reduction(MpyIter *iter, int iop)
{
    npy_uint32 itflags = NIT_ITFLAGS(iter);
                    ...
                    npy_intp *itershape)
{
    ...
    NpyIter_AxisData *axisdata;
    npy_intp sizeof_axisdata;
    PyMicArrayObject **op = NIT_OPERANDS(iter), *op_cur;
    ...
    for (iop = 0; iop < nop; ++iop) {
        op_cur = op[iop];
        if (op_cur != NULL) {
            npy_intp *shape = PyMicArray_DIMS(op_cur);
            ondim = PyMicArray_NDIM(op_cur);
        ...
        for (iop = 0; iop < nop; ++iop) {
            ...
            if (op_axes == NULL || op_axes[iop] == NULL) {
                    ...
                    if (bshape == 1) {
                        ...
                        if (idim >= ondim &&
                            ...
                            goto operand_different_than_broadcast;
                                    ...
                                    PyMicArray_DIM(op_cur, ondim-idim-1) == 1) {
                        strides[iop] = 0;
                        ...
                        strides[iop] = PyMicArray_STRIDE(op_cur, ondim-idim-1);
                    }
                }
            }
            else {
                ...
                if (i >= 0) {
                    if (bshape == 1 || op_cur == NULL) {
                    ...
                    else if (PyMicArray_DIM(op_cur, i) == 1) {
                        strides[iop] = 0;
                        if (op_flags[iop] & NPY_ITER_NO_BROADCAST) {
                            goto operand_different_than_broadcast;
                        }
                        ...
                        strides[iop] = PyMicArray_STRIDE(op_cur, i);
                    }
                }
                else if (bshape == 1) {
                    strides[iop] = 0;
...
broadcast_error: {
        ...
        if (op_axes == NULL) {
            ...
            for (iop = 0; iop < nop; ++iop) {
                if (op[iop] != NULL) {
                    tmp = convert_shape_to_string(PyMicArray_NDIM(op[iop]),
                                                    PyMicArray_DIMS(op[iop]),
                                                    " ");
                    if (tmp == NULL) {
                        Py_DECREF(errmsg);
                        return 0;
                ...
                if (op[iop] != NULL) {
                    ...
                    if (axes != NULL) {
                        for (idim = 0; idim < ndim; ++idim) {
                            ...
                            if (i >= 0 && i < PyMicArray_NDIM(op[iop])) {
                                remdims[idim] = PyMicArray_DIM(op[iop], i);
                            }
                            else {
                                remdims[idim] = -1;
                            }
                      ...
                      int *op_axes)
{
    ...
    if (op_axes != NULL) {
        for (idim = 0; idim < ndim; ++idim, NIT_ADVANCE_AXISDATA(axisdata, 1)) {
            ...
            if (0 <= i && i < op_ndim) {
                shape = PyMicArray_DIM(op, i);
                if (shape != 1) {
                    npy_intp stride = PyMicArray_STRIDE(op, i);
                    if (p < 0) {
    ...
    case NPY_ANYORDER:
        ...
        if (ndim > 1) {
            ...
            for (iop = 0; iop < nop; ++iop, ++op) {
                if (*op && !PyMicArray_CHKFLAGS(*op, NPY_ARRAY_F_CONTIGUOUS)) {
                   forder = 0;
                   break;
                }
            }
                        ...
                        int only_inputs)
{
    ...
    PyMicArrayObject *arrs[NPY_MAXARGS];
    ...
    if (narrs == 0) {
        ...
        if (i == ndtypes) {
            ...
            ret = PyArray_ResultType(narrs, (PyArrayObject **)arrs, ndtypes, dtypes);
        }
    }
    else {
        ret = PyArray_ResultType(narrs, (PyArrayObject **)arrs, ndtypes, dtypes);
                ...
                PyArray_Descr *op_dtype, int *op_axes)
{
    ...
    if (op_ndim == 0) {
        Py_INCREF(op_dtype);
        ret = (PyMicArrayObject *)PyMicArray_NewFromDescr(device,
                               subtype, op_dtype, 0,
    ...
    if (subtype != &PyMicArray_Type) {
        if (PyMicArray_NDIM(ret) != op_ndim ||
                    !PyArray_CompareLists(shape, PyMicArray_DIMS(ret), op_ndim)) {
        ...
        for (iop = 0; iop < nop; ++iop) {
            ...
            for (iother = 0; iother < nop; ++iother) {
                ...
                if ((op_flags[iop] & NPY_ITER_OVERLAP_ASSUME_ELEMENTWISE) &&
                    ...
                    PyMicArray_BYTES(op[iop]) == PyMicArray_BYTES(op[iother]) &&
                    PyMicArray_NDIM(op[iop]) == PyMicArray_NDIM(op[iother]) &&
                    PyArray_CompareLists(PyMicArray_DIMS(op[iop]),
                                         PyMicArray_DIMS(op[iother]),
                                         PyMicArray_NDIM(op[iop])) &&
                    PyArray_CompareLists(PyMicArray_STRIDES(op[iop]),
                                         PyMicArray_STRIDES(op[iother]),
                                         PyMicArray_NDIM(op[iop])) &&
                    PyMicArray_DESCR(op[iop]) == PyMicArray_DESCR(op[iother]) &&
                    solve_may_have_internal_overlap((PyArrayObject *)op[iop], 1) == 0) {
                 ...
                 */
                may_share_memory = solve_may_share_memory((PyArrayObject *)op[iop],
                                                          (PyArrayObject *)op[iother],
                                                          1);
    ...
    for (iop = 0; iop < nop; ++iop) {
        ...
        if (op[iop] == NULL) {
            ...
            npyiter_replace_axisdata(iter, iop, op[iop], ondim,
                    PyMicArray_DATA(op[iop]), op_axes ? op_axes[iop] : NULL);
         ...
         */
            ...
            temp = (PyMicArrayObject *)PyMicArray_NewFromDescr(device,
                                        &PyMicArray_Type, op_dtype[iop],
                                        0, NULL, NULL, NULL, 0, NULL);
            if (temp == NULL) {
                return 0;
            }
            if (PyMicArray_CopyInto(temp, op[iop]) != 0) {
                Py_DECREF(temp);
                return 0;
            }
            Py_DECREF(op[iop]);
            ...
            op_itflags[iop] &= ~NPY_OP_ITFLAG_CAST;
            if (itflags & NPY_ITFLAG_BUFFER) {
                //TODO: change in buffer mechanism
         ...
         */
                 ...
                 (op_itflags[iop] & NPY_OP_ITFLAG_FORCECOPY)) {
            PyMicArrayObject *temp;
            int ondim = PyMicArray_NDIM(op[iop]);

            /* Allocate the temporary array, if possible */
            temp = npyiter_new_temp_array(iter, &PyMicArray_Type,
                                        flags, &op_itflags[iop],
                                        ondim,
                                        PyMicArray_DIMS(op[iop]),
                                        op_dtype[iop],
            ...
            if (op_itflags[iop] & NPY_OP_ITFLAG_WRITE) {
                Py_INCREF(op[iop]);
                if (PyArray_SetUpdateIfCopyBase((PyArrayObject *)temp,
                            (PyArrayObject *)op[iop]) < 0) {
                    Py_DECREF(temp);
                    return 0;
                }
            }
             ...
             */
            npyiter_replace_axisdata(iter, iop, op[iop], ondim,
                    PyMicArray_BYTES(op[iop]), op_axes ? op_axes[iop] : NULL);
             ...
             * optimizations.
             */
            if (PyMicArray_ISALIGNED(op[iop])) {
                op_itflags[iop] |= NPY_OP_ITFLAG_ALIGNED;
            }
        }
        ...
        if ((itflags & NPY_ITFLAG_BUFFER) &&
            ...
            else if (PyMicArray_NDIM(op[iop]) > 0) {
        ...
        if (!(flags & NPY_OP_ITFLAG_BUFNEVER)) {
            ...
            if (flags & NPY_OP_ITFLAG_WRITE) {
                ...
                if (flags & NPY_OP_ITFLAG_WRITEMASKED) {
                    int maskop = NIT_MASKOP(iter);
                    PyArray_Descr *mask_dtype = PyMicArray_DESCR(op[maskop]);
                     ...
                     */
                    if (PyMicArray_GetMaskedDTypeTransferFunction(
                                (flags & NPY_OP_ITFLAG_ALIGNED) != 0,
                                op_dtype[iop]->elsize,
                                op_stride,
