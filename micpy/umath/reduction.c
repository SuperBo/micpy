...
allocate_reduce_result(PyMicArrayObject *arr, npy_bool *axis_flags,
                        PyArray_Descr *dtype, int subok)
{
    npy_intp strides[NPY_MAXDIMS], stride;
    npy_intp shape[NPY_MAXDIMS], *arr_shape = PyMicArray_DIMS(arr);
...
conform_reduce_result(int ndim, npy_bool *axis_flags,
                      PyMicArrayObject *out, int keepdims, const char *funcname,
                      int need_copy)
{
    npy_intp strides[NPY_MAXDIMS], shape[NPY_MAXDIMS];
    npy_intp *strides_out = PyMicArray_STRIDES(out);
    npy_intp *shape_out = PyMicArray_DIMS(out);
    int idim, idim_out, ndim_out = PyMicArray_NDIM(out);
    PyArray_Descr *dtype;
    ...
    Py_INCREF(out);
    if (PyMicArray_SetBaseObject((PyMicArrayObject *)ret, (PyObject *)out) < 0) {
        Py_DECREF(ret);
        return NULL;
    }
    ...
    if (need_copy) {
        ...
        if (PyMicArray_CopyInto(ret_copy, (PyMicArrayObject *)ret) != 0) {
            Py_DECREF(ret);
            Py_DECREF(ret_copy);
            return NULL;
        }
        ...
        Py_INCREF(ret);
        if (PyMicArray_SetUpdateIfCopyBase(ret_copy, (PyMicArrayObject *)ret) < 0) {
            Py_DECREF(ret);
            Py_DECREF(ret_copy);
            return NULL;
        }
...
PyMicArray_CreateReduceResult(PyMicArrayObject *operand, PyMicArrayObject *out,
                           ...
                           const char *funcname)
{
    ...
    if (out == NULL) {
        ...
        Py_XDECREF(dtype);
        result = conform_reduce_result(PyMicArray_NDIM(operand), axis_flags,
 ...
 */
NPY_NO_EXPORT PyMicArrayObject *
PyMicArray_InitializeReduceResult(
                    PyMicArrayObject *result, PyMicArrayObject *operand,
                    npy_bool *axis_flags, int reorderable,
                    npy_intp *out_skip_first_count, const char *funcname)
{
    npy_intp *strides, *shape, shape_orig[NPY_MAXDIMS];
    PyMicArrayObject *op_view = NULL;
    int idim, ndim, nreduce_axes;

    ndim = PyMicArray_NDIM(operand);
    ...
    *out_skip_first_count = 0;
    ...
    op_view = (PyMicArrayObject *)PyMicArray_View(operand, NULL, &PyMicArray_Type);
    if (op_view == NULL) {
        return NULL;
    }
     ...
     * at the same time.
     */
    shape = PyMicArray_SHAPE(op_view);
    nreduce_axes = 0;
     ...
     */
    if (PyMicArray_CopyInto(result, op_view) < 0) {
        Py_DECREF(op_view);
        return NULL;
    }
     ...
     */
    if (nreduce_axes == 1) {
        strides = PyMicArray_STRIDES(op_view);
        for (idim = 0; idim < ndim; ++idim) {
            if (axis_flags[idim]) {
                shape[idim] = shape_orig[idim] - 1;
                ((PyMicArrayObject *)op_view)->data += strides[idim];
            }
        }
    }
     ...
     */
        ...
        *out_skip_first_count = PyMicArray_SIZE(result);
                      ...
                      PyUFuncObject *ufunc, npy_intp buffersize, const char *funcname)
{
    ...
    result = PyMicArray_CreateReduceResult(operand, out,
    ...
    if (assign_identity != NULL) {
         ...
         * only 0 or 1 axes in axis_flags.
         */
        if (!reorderable && check_nonreorderable_axes(PyMicArray_NDIM(operand),
                                        axis_flags, funcname) < 0) {
            goto fail;
        }
        ...
        op_view = PyMicArray_InitializeReduceResult(result, operand,
        ...
        if ((PyMicArray_SIZE(op_view) == 0) || (PyMicArray_NDIM(operand) == 0)) {
            Py_DECREF(op_view);
            op_view = NULL;
            goto finish;
        }
...
finish:
    ...
    if (out == NULL) {
        if (!keepdims) {
            PyMicArray_RemoveAxesInPlace(result, axis_flags);
        }
    }
    else {
        Py_DECREF(result);
