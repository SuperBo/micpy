...
float mpy_half_to_float(npy_half h)
{
    ...
    conv.retbits = mpy_halfbits_to_floatbits(h);
    return conv.ret;
}
...
double mpy_half_to_double(npy_half h)
{
    ...
    conv.retbits = mpy_halfbits_to_doublebits(h);
    return conv.ret;
}
...
npy_half mpy_float_to_half(float f)
{
    ...
    conv.f = f;
    return mpy_floatbits_to_halfbits(conv.fbits);
}
...
npy_half mpy_double_to_half(double d)
{
    ...
    conv.d = d;
    return mpy_doublebits_to_halfbits(conv.dbits);
}
...
npy_half mpy_half_nextafter(npy_half x, npy_half y)
{
    ...
    if (!mpy_half_isfinite(x) || mpy_half_isnan(y)) {
#if NPY_HALF_GENERATE_INVALID
        mpy_set_floatstatus_invalid();
#endif
        ret = NPY_HALF_NAN;
    } else if (mpy_half_eq_nonan(x, y)) {
        ret = x;
    } else if (mpy_half_iszero(x)) {
...
int mpy_half_eq(npy_half h1, npy_half h2)
{
     ...
     */
    return (!mpy_half_isnan(h1) && !mpy_half_isnan(h2)) &&
           (h1 == h2 || ((h1 | h2) & 0x7fff) == 0);
}

int mpy_half_ne(npy_half h1, npy_half h2)
{
    return !mpy_half_eq(h1, h2);
}
...
{
    return (!mpy_half_isnan(h1) && !mpy_half_isnan(h2)) && mpy_half_lt_nonan(h1, h2);
}

int mpy_half_gt(npy_half h1, npy_half h2)
{
    return mpy_half_lt(h2, h1);
}
...
{
    return (!mpy_half_isnan(h1) && !mpy_half_isnan(h2)) && mpy_half_le_nonan(h1, h2);
}

int mpy_half_ge(npy_half h1, npy_half h2)
{
    return mpy_half_le(h2, h1);
}

npy_half mpy_half_divmod(npy_half h1, npy_half h2, npy_half *modulus)
{
    float fh1 = mpy_half_to_float(h1);
    float fh2 = mpy_half_to_float(h2);
    float div, mod;

    div = mpy_divmodf(fh1, fh2, &mod);
    *modulus = mpy_float_to_half(mod);
    return mpy_float_to_half(div);
}
